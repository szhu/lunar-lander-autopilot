...
const getVectorVelocity = (velocity)=>Math.sqrt(Math.pow(velocity.x, 2) + Math.pow(velocity.y, 2));
...
const velocityInMPH = (velocity)=>Intl.NumberFormat().format((getVectorVelocity(velocity) * 20).toFixed(1));
const heightInFeet = (yPos, groundedHeight)=>Intl.NumberFormat().format(Math.abs(Math.round((yPos - groundedHeight) / 3.5)));
const progress = (start, end, current)=>(current - start) / (end - start);
const percentProgress = (start, end, current)=>Math.max(0, Math.min((current - start) / (end - start) * 100, 100));
const simpleBallisticUpdate = (currentPosition, currentVelocity, currentAngle, groundedHeight, rotationDirection, currentRotationVelocity, canvasWidth)=>{
    const newPosition = {
        ...currentPosition
    };
    const newVelocity = {
    ...
    newPosition.y = Math.min(currentPosition.y + currentVelocity.y, groundedHeight + 1);
    newPosition.x = currentPosition.x + currentVelocity.x;
    if (newPosition.y <= groundedHeight) {
        newRotationVelocity = rotationDirection ? currentRotationVelocity + randomBetween(0, 0.01) : currentRotationVelocity - randomBetween(0, 0.01);
...
const makeExplosion = (state, position, velocity, angle)=>{
    ...
    const _makeLanderChunk = (drawInstructions, yOffset)=>{
        ...
        let _angle = 0;
        const draw = ()=>{
            [_position, _velocity, _rotationVelocity, _angle] = simpleBallisticUpdate(_position, _velocity, _angle, _groundedHeight, _rotationDirection, _rotationVelocity, canvasWidth);
            CTX.save();
    ...
    const smallExplosionChunks = new Array(32).fill().map(()=>_makeRandomExplosionPiece(position, velocity));
    const draw = ()=>{
        noseCone.draw();
        chunk1.draw();
        chunk2.draw();
...
const makeConfetti = (state, amount, passedPosition, passedVelocity)=>{
    ...
    const _startingPosition = (index)=>passedPosition ? passedPosition : {
            x: canvasWidth / 2 + index - confettiTypeAmount / 2,
            y: canvasHeight / 2
        };
    const _startingVelocity = (index)=>passedVelocity ? {
            x: index < confettiTypeAmount / 2 ? passedVelocity.x - 1 : passedVelocity.x + 1,
    ...
    const _makeTwirlPiece = (position, velocity)=>{
        ...
        const mirroredLoopingProgress = (start, end, current)=>{
            ...
            return Math.floor(current / end) % 2 ? Math.abs(loopedProgress - 1) : loopedProgress;
        };
        const draw = ()=>{
            [_position, _velocity, _rotationVelocity] = simpleBallisticUpdate(_position, _velocity, 0, _groundedHeight, _rotationDirection, _rotationVelocity, canvasWidth);
            const width = mirroredLoopingProgress(0, 0.1, Math.abs(_rotationVelocity)) * _size;
            CTX.save();
            CTX.fillStyle = _color;
            CTX.translate(_position.x, _position.y);
            CTX.beginPath();
    ...
    const confettiPieces = new Array(confettiTypeAmount).fill().map((_, index)=>_makeConfettiPiece(_startingPosition(index), _startingVelocity(index)));
    const twirlPieces = new Array(confettiTypeAmount).fill().map((_, index)=>_makeTwirlPiece(_startingPosition(index), _startingVelocity(index)));
    const draw = ()=>{
        if (!hasPlayedAudio) {
            audio.playConfetti();
            hasPlayedAudio = true;
...
const landingScoreDescription = (score)=>{
    const description = score >= 99 ? "Perfect landing, incredible, you can’t get better than this" : score >= 95 ? "Near-perfect landing, super smooth" : score >= 90 ? "Very nice landing, amazing" : score >= 85 ? "Pretty good landing, keep going!" : score >= 80 ? "A good landing, keep trying" : score >= 75 ? "Just shy of a good landing" : score >= 70 ? "A solid “C” landing" : score >= 65 ? "You landed but it could have been slower and straighter" : score >= 60 ? "Not the worst landing, but not very good either" : score >= 55 ? "Pretty bad landing, but it could be worse" : score >= 55 ? "Not great" : score >= 40 ? "Basically a fender bender, but you landed" : score >= 30 ? "A near-crash, way too fast" : "Terrible landing, you need to land slow and straight";
    return description;
};
const crashScoreDescription = (score)=>{
    const description = score >= 100 ? "Unbelievable, the crater is visible from Earth" : score >= 95 ? "Ludicrous crash! The debris has entered orbit" : score >= 90 ? "Incredible crash, the lander has vaporized" : score >= 85 ? "Impressive speed, impressive angle - you crashed with style" : score >= 80 ? "A fast crash, but it could be faster" : score >= 75 ? "I think you meant to do that" : score >= 70 ? "You definitely did not land…" : score >= 65 ? "I think there’s a problem with the lander" : score >= 60 ? "Sick crash!" : score >= 50 ? "Were you trying to land, or…" : score >= 40 ? "A bad crash, but it could be worse" : score >= 30 ? "I don’t think we’re getting back to Earth" : score >= 20 ? "A smooth… wait… you crashed" : score >= 10 ? "The lander has been… damaged" : "So, so close to a landing, but still a crash";
    return description;
};
...
const drawTrajectory = (state, currentPosition, currentAngle, currentVelocity, currentRotationVelocity, groundedHeight)=>{
    const CTX = state.get("CTX");
    const canvasHeight = state.get("canvasHeight");
    ...
    while(projectedYPosition < groundedHeight){
        projectedYPosition = Math.min(projectedYPosition + projectedYVelocity, groundedHeight);
...
const makeLander = (state, onGameEnd, onResetXPos)=>{
    ...
    const resetProps = ()=>{
        ...
        _velocity = {
            x: randomBetween(-_thrust * (canvasWidth / 10), _thrust * (canvasWidth / 10)),
    ...
    const _setGameEndData = (landed, timeSinceStart)=>{
        _gameEndData = {
            ...
            durationInSeconds: Intl.NumberFormat().format(Math.round(timeSinceStart / 1000)),
        ...
        if (landed) {
            ...
            _gameEndData.explosion = makeExplosion(state, _position, _velocity, _angle);
        }
    ...
    const _updateProps = (timeSinceStart)=>{
        ...
        if (_position.y < _groundedHeight) {
            ...
            } else if (_boostersUsedPreviousFrame && !_rotatingLeft && !_rotatingRight) {
                _boostersUsedPreviousFrame = false;
            }
            ...
            _setGameEndData(getVectorVelocity(_velocity) < 0.6 && getAngleDeltaUpright(_angle) < 11, timeSinceStart);
        }
    };
    const _drawHUD = ()=>{
        const textWidth = CTX.measureText("100.0 MPH").width + 2;
        const staticPosition = getVectorVelocity(_velocity) > 7;
        const xPosBasis = staticPosition ? canvasWidth / 2 - textWidth / 2 : Math.min(_position.x + 20 * 2, canvasWidth - textWidth);
        const yPosBasis = Math.max(_position.y, 30);
        const rotatingLeft = _rotationVelocity < 0;
        const speedColor = getVectorVelocity(_velocity) > 0.6 ? "rgb(255, 0, 0)" : "rgb(0, 255, 0)";
        const angleColor = getAngleDeltaUpright(_angle) > 11 ? "rgb(255, 0, 0)" : "rgb(0, 255, 0)";
        CTX.save();
        ...
        CTX.fillText(`${heightInFeet(_position.y, _groundedHeight)} FT`, xPosBasis, yPosBasis + 14);
        CTX.restore();
    ...
    const draw = (timeSinceStart)=>{
        ...
        if (!_engineOn && !_rotatingLeft && !_rotatingRight) {
            drawTrajectory(state, _position, _angle, _velocity, _rotationVelocity, _groundedHeight);
        }
...
const makeToyLander = (state, onEngineOn, onLeftRotation, onRightRotation, onEngineAndRotation)=>{
    const CTX = state.get("CTX");
    const canvasWidth = state.get("canvasWidth");
    const canvasHeight = state.get("canvasHeight");
    ...
    const draw = ()=>{
        ...
        if (_engineOn) {
            const _flameHeight = randomBetween(_toyLanderEngineLengthMin, _toyLanderEngineLengthMax);
        ...
        const _boosterLength = randomBetween(_toyLanderBoosterLengthMin, _toyLanderBoosterLengthMax);
        if (_rotatingLeft) {
            CTX.beginPath();
...
const makeControls = (state, lander, audioManager)=>{
    ...
    const getTouchZone = (x)=>{
        const clampedColumnNumber = Math.max(0, Math.min(Math.floor(x / (canvasWidth / touchColumnMap.length)), touchColumnMap.length));
        return touchColumnMap[clampedColumnNumber];
    };
    const getColumnBoundary = (colName)=>{
        const start = touchColumnMap.findIndex((e)=>e === colName) / touchColumnMap.length;
        const end = (touchColumnMap.findLastIndex((e)=>e === colName) + 1) / touchColumnMap.length;
        return {
...
const showStatsAndResetControl = (lander, animationObject, data, hasKeyboard)=>{
...
https://ehmorris.com/lander/`;
    const hideStats = ()=>{
        document.querySelector("#endGameStats .buttonContainer").classList.remove("show");
    ...
    const populateMeter = (name, percentPosition, textValue)=>{
        ...
        setTimeout(()=>{
            meter.querySelector("[data-percent-position]").style.left = `${percentPosition}%`;
        }, 0);
    };
    ...
    const populateStats = (data)=>{
        ...
        document.querySelector("#type").textContent = data.landed ? "landing" : "crash";
    ...
    const attachEventListeners = ()=>{
        ...
        if (canShowShareSheet) {
            document.querySelector("#share").addEventListener("click", showShareSheet);
        } else if (canCopyText) {
            document.querySelector("#copyText").addEventListener("click", copyShareStats);
        }
        if (hasKeyboard) {
            setTimeout(()=>{
    ...
    const detachEventListeners = ()=>{
        ...
        if (canShowShareSheet) {
            document.querySelector("#share").removeEventListener("click", showShareSheet);
        }
        if (hasKeyboard) {
...
const manageInstructions = (onCloseInstructions)=>{
    ...
    const show = ()=>{
        ...
        document.querySelector("#crashSpeed").textContent = `${CRASH_VELOCITY * VELOCITY_MULTIPLIER} MPH`;
    ...
    const checkDone = ()=>{
        if (_engineDone && _leftRotationDone && _rightRotationDone && _engineAndRotationDone) {
    ...
    const setLeftRotationDone = ()=>{
        ...
        document.querySelector("#rightRotationCheck").classList.add("strikethrough");
        checkDone();
    };
    ...
    const setEngineAndRotationDone = ()=>{
        ...
        document.querySelector("#engineAndRotationCheck").classList.add("strikethrough");
        checkDone();
    };
    return {
        show,
...
const makeAudioManager = ()=>{
    ...
    document.addEventListener("keydown", ({ isTrusted , metaKey , shiftKey , ctrlKey , altKey , key  })=>{
...
const makeTallyManger = ()=>{
    ...
    const getLandingTotalStorage = ()=>{
        try {
            return localStorage.getItem("landingTotal") ? localStorage.getItem("landingTotal") : 0;
        } catch  {
    ...
    const getCrashTotalStorage = ()=>{
        try {
            return localStorage.getItem("crashTotal") ? localStorage.getItem("crashTotal") : 0;
        } catch  {
            return _crashTotal;
        }
    };
    ...
    const updateDisplay = ()=>{
        document.querySelector("#landingTotal").textContent = getLandingTotalStorage();
...
const appState = makeStateManager().set("CTX", CTX).set("canvasWidth", canvasWidth).set("canvasHeight", canvasHeight).set("canvasElement", canvasElement).set("audioManager", audioManager);
const instructions = manageInstructions(onCloseInstructions);
const toyLander = makeToyLander(appState, ()=>instructions.setEngineDone(), ()=>instructions.setLeftRotationDone(), ()=>instructions.setRightRotationDone(), ()=>instructions.setEngineAndRotationDone());
...
function onGameEnd(data) {
    showStatsAndResetControl(lander, animationObject, data, landerControls.getHasKeyboard());
    if (data.landed) {
...
document.addEventListener("keydown", ({ key  })=>{
    if (key === "c") {
        randomConfetti.push(makeConfetti(appState, 10, {
